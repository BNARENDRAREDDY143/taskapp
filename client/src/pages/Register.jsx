...
import { CheckSquare, Lock, Mail, User, Eye, EyeOff, Sparkles, Moon, Sun, ArrowRight, ArrowLeft } from 'lucide-react';
import GlowingWave from '../components/GlowingWave';

const Register = () => {
  const navigate = useNavigate();
  const { register, demoLogin, loading, authError, setAuthError } = useAuth();
      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800/80 shadow-2xl space-y-6 relative z-10 animate-fade-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Join TaskPro to organize projects, milestones, and Kanban boards
          </p>
        </div>

        {/* Demo Shortcut */}
        <div className="p-3 rounded-2xl bg-indigo-50/80 dark:bg-indigo-950/40 border border-indigo-200/80 dark:border-indigo-800/80 text-center space-y-2">
          <button
            type="button"
            onClick={handleDemo}
            disabled={loading}
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white placeholder:text-slate-400"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2 text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white placeholder:text-slate-400"
              />
              <button
                type="button"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-2.5 px-4 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 disabled:opacity-50 rounded-xl shadow-md shadow-indigo-500/25 transition-all"
          >
